...
import { Blob, RevealCard, GlassCard, StepBadge, AppStoreBadge } from '../../../../core/components/brand/BrandUI';
...
const HomePage: React.FC = () => {
    ...
    return (
        ...
        >
            {/* ══ HERO ══ */}
            <Box sx={{ position: 'relative', overflow: 'hidden', pb: { xs: 8, md: 14 }, pt: { xs: 5, md: 10 } }}>
                <Blob sx={{ width: 480, height: 480, background: 'rgba(199,91,18,0.28)', top: -120, right: -100 }} />
                <Blob sx={{ width: 340, height: 340, background: 'rgba(255,176,103,0.18)', bottom: -60, left: -80, animationDelay: '2s' }} />
                <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
                    {/* Nav */}
                    <Stack direction="row" justifyContent="space-between" alignItems="center" sx={{ mb: { xs: 6, md: 10 } }}>
                        <Box component="img" src={logoWithText} alt="KalyNow" sx={{ height: 40, filter: 'brightness(1.1)', animation: 'floatY 5s ease-in-out infinite' }} />
                        <Stack direction="row" spacing={1.5}>
                            <Button size="small" onClick={() => navigate('/restaurants')} sx={{ color: 'rgba(255,255,255,0.75)', '&:hover': { color: BRAND.secondary } }}>Restaurants</Button>
                            <Button size="small" onClick={() => navigate('/offers')} sx={{ color: 'rgba(255,255,255,0.75)', '&:hover': { color: BRAND.secondary } }}>Offres</Button>
                            <Button variant="outlined" size="small" onClick={() => navigate('/login')}
                                sx={{ borderColor: BRAND.secondary, color: BRAND.secondary, borderRadius: 8, '&:hover': { bgcolor: 'rgba(255,176,103,0.1)' } }}>
                                Connexion
                            </Button>
                        </Stack>
                    </Stack>
                    ...
                    <Box ref={heroRef} sx={{ textAlign: 'center', opacity: 0, transform: 'translateY(24px)', transition: 'opacity .75s ease, transform .75s ease' }}>
                        <Chip label="Zéro gaspillage · 100 % bon plan"
                            sx={{ mb: 3, bgcolor: 'rgba(255,176,103,0.15)', color: BRAND.secondaryLight, fontWeight: 600, fontSize: 13 }} />
                        <Typography component="h1" sx={{
                            fontSize: { xs: '2.4rem', md: '4rem' }, fontWeight: 900, lineHeight: 1.15, mb: 3,
                            background: BRAND.shimmer, backgroundSize: '200% auto',
                            WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', backgroundClip: 'text',
                            animation: 'shimmer 6s linear infinite',
                        }}>
                            Sauvez un repas.<br />Faites une bonne affaire.
                        </Typography>
                        <Typography variant="h6" sx={{ color: BRAND.textMuted, maxWidth: 640, mx: 'auto', mb: 5, lineHeight: 1.75 }}>
                        ...
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} justifyContent="center">
                            ...
                            <Button variant="outlined" size="large" onClick={() => navigate('/register')}
                                sx={{ px: 4, py: 1.5, fontSize: 16, fontWeight: 600, borderRadius: 10, borderColor: 'rgba(255,176,103,0.6)', color: BRAND.secondary, backdropFilter: 'blur(10px)', transition: 'transform .25s ease, border-color .25s ease', '&:hover': { transform: 'translateY(-4px)', borderColor: BRAND.secondary, bgcolor: 'rgba(255,176,103,0.08)' } }}>
            ...
            {/* ══ STATS ══ */}
            <Box sx={{ background: 'rgba(199,91,18,0.1)', borderTop: '1px solid rgba(255,176,103,0.12)', borderBottom: '1px solid rgba(255,176,103,0.12)', py: 5 }}>
                <Container maxWidth="lg">
                    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-around" alignItems="center" spacing={4}
                        divider={<Divider orientation="vertical" flexItem sx={{ borderColor: 'rgba(255,176,103,0.2)' }} />}>
            ...
            <Container maxWidth="md" sx={{ py: { xs: 8, md: 14 }, position: 'relative' }}>
                <Blob sx={{ width: 380, height: 380, background: 'rgba(199,91,18,0.14)', top: 0, right: -180 }} />
                <RevealCard>
                ...
                <Stack spacing={0} sx={{ position: 'relative', zIndex: 1 }}>
                    <Box sx={{ position: 'absolute', left: 31, top: 12, bottom: 12, width: 2, background: 'linear-gradient(to bottom, rgba(255,176,103,0.6), rgba(199,91,18,0.1))', zIndex: 0 }} />
                ...
                <RevealCard delay={450}>
                    <Paper elevation={0} sx={{ mt: 2, p: { xs: 3, md: 4 }, borderRadius: 4, textAlign: 'center', background: 'linear-gradient(135deg, rgba(199,91,18,0.25) 0%, rgba(255,176,103,0.12) 100%)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: `1px solid ${BRAND.glassBorder}` }}>
            ...
            {/* ══ SECTION RESTAURANTS ══ */}
            <Box sx={{ background: 'rgba(255,255,255,0.025)', borderTop: '1px solid rgba(255,176,103,0.1)', borderBottom: '1px solid rgba(255,176,103,0.1)', py: { xs: 8, md: 14 }, position: 'relative', overflow: 'hidden' }}>
                <Blob sx={{ width: 420, height: 420, background: 'rgba(199,91,18,0.18)', bottom: -100, left: -120, animationDelay: '3s' }} />
                <Container maxWidth="lg" sx={{ position: 'relative', zIndex: 1 }}>
                    <RevealCard>
                        <Box sx={{ textAlign: 'center', mb: 8 }}>
                            <Chip label="Vous êtes restaurateur ?" sx={{ mb: 2, bgcolor: 'rgba(255,176,103,0.15)', color: BRAND.secondaryLight, fontWeight: 600 }} />
                    ...
                    <Stack direction={{ xs: 'column', sm: 'row' }} flexWrap="wrap" gap={3} sx={{ mb: 8 }}>
                        {AVANTAGES_RESTO.map(({ icon, titre, texte }, i) => (
                            <GlassCard key={titre} icon={icon} titre={titre} texte={texte} delay={i * 100} />
                        ))}
                    </Stack>
                    <RevealCard delay={200}>
                        <Paper elevation={0} sx={{ p: { xs: 3, md: 5 }, borderRadius: 5, background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(20px)', WebkitBackdropFilter: 'blur(20px)', border: 'solid 1px rgba(255,176,103,0.2)' }}>
                            ...
                            <Stack direction={{ xs: 'column', md: 'row' }} spacing={4} divider={<Divider orientation="vertical" flexItem sx={{ borderColor: 'rgba(255,176,103,0.15)' }} />}>
                                {[
                                    ...
                                    <Box key={num} sx={{ flex: 1 }}>
                                        <Typography sx={{ fontSize: '2.4rem', fontWeight: 900, color: 'transparent', WebkitTextStroke: '2px rgba(255,176,103,0.45)', lineHeight: 1, mb: 1 }}>{num}</Typography>
                ...
                <Stack direction={{ xs: 'column', md: 'row' }} spacing={3}>
                    {TEMOIGNAGES.map(({ nom, role, texte }, i) => (
                        <RevealCard key={nom} delay={i * 120} sx={{ flex: 1 }}>
                            <Paper elevation={0} sx={{ p: 4, height: '100%', borderRadius: 4, background: 'rgba(255,255,255,0.04)', backdropFilter: 'blur(16px)', WebkitBackdropFilter: 'blur(16px)', border: `1px solid ${BRAND.glassBorder}`, transition: 'transform .3s ease, border-color .3s ease', '&:hover': { transform: 'translateY(-6px)', borderColor: BRAND.glassBorderHover } }}>
                                <Typography variant="body1" sx={{ color: 'rgba(255,255,255,0.82)', lineHeight: 1.8, fontStyle: 'italic', mb: 3 }}>{texte}</Typography>
                                <Divider sx={{ borderColor: 'rgba(255,176,103,0.18)', mb: 2 }} />
            ...
            {/* ══ CTA FINAL ══ */}
            <Box sx={{ position: 'relative', overflow: 'hidden', py: { xs: 10, md: 16 }, textAlign: 'center' }}>
                <Blob sx={{ width: 500, height: 500, background: 'rgba(199,91,18,0.22)', top: '50%', left: '50%', transform: 'translate(-50%,-50%)' }} />
                    ...
                    <RevealCard>
                        ...
                        <Stack direction={{ xs: 'column', sm: 'row' }} spacing={2} justifyContent="center">
                            ...
                            <Button variant="outlined" size="large" onClick={() => navigate('/register?type=restaurant')}
                                sx={{ px: 4, py: 1.8, fontSize: 16, fontWeight: 700, borderRadius: 12, borderColor: 'rgba(255,176,103,0.5)', color: BRAND.secondary, transition: 'transform .3s ease', '&:hover': { borderColor: BRAND.secondary, bgcolor: 'rgba(255,176,103,0.08)', transform: 'scale(1.02)' } }}>
            ...
            {/* ══ FOOTER ══ */}
            <Box sx={{ borderTop: '1px solid rgba(255,176,103,0.1)', py: 4, textAlign: 'center' }}>
